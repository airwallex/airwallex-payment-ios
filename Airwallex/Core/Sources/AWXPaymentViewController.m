...
- (void)viewDidLoad {
    ...
    _totalLabel.text = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"Total %@", nil, [NSBundle resourceBundle], nil), [self.session.amount stringWithCurrencyCode:self.session.currency]];
    ...
    _cvcField.placeholder = NSLocalizedStringFromTableInBundle(@"CVC / CVV", nil, [NSBundle resourceBundle], @"CVC / CVV");
    ...
    [_confirmButton setTitle:NSLocalizedStringFromTableInBundle(@"Pay now", nil, [NSBundle resourceBundle], @"Pay now") forState:UIControlStateNormal];
