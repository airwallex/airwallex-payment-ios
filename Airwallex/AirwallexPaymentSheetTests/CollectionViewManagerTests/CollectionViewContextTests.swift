...
@MainActor class CollectionViewContextTests: XCTestCase {
    ...
    func testApplySnapshot() {
        ...
        snapshot.appendItems([CompoundItem(.A, .B1), CompoundItem(.A, .B2)], toSection: .A)
        ...
        XCTAssertEqual(appliedSnapshot.itemIdentifiers(inSection: .A), [CompoundItem(.A, .A1), CompoundItem(.A, .A2), CompoundItem(.A, .B1), CompoundItem(.A, .B2)])
    }
    ...
    func testInvalidateLayoutForItems() {
        ...
        context.invalidateLayout(for: [CompoundItem(.B, .B2)])
        mockManager.collectionView.layoutIfNeeded()
    ...
    func testReloadItems() {
        ...
        // Reload sections
        context.reload(items: [CompoundItem(.A, .A1)])
        mockManager.collectionView.layoutIfNeeded()
        
        // Verify that the appropriate methods were called
        XCTAssertEqual(mockProvider.sectionControllerA.cellForItemAtIndexPathCalled?.sectionItem.item, Item.A1)
        XCTAssertEqual(mockProvider.sectionControllerA.cellForItemAtIndexPathCalled?.indexPath, IndexPath(item: 0, section: 0))
        XCTAssertEqual(mockProvider.sectionControllerA.didEndDisplayingCellCalled?.sectionItem.item, Item.A1)
        XCTAssertEqual(mockProvider.sectionControllerA.didEndDisplayingCellCalled?.indexPath, IndexPath(item: 0, section: 0))
    
        XCTAssertEqual(mockProvider.sectionControllerA.willDisplayCellCalled?.sectionItem.item, Item.A1)
        XCTAssertEqual(mockProvider.sectionControllerA.willDisplayCellCalled?.indexPath, IndexPath(item: 0, section: 0))
    }
    ...
    func testReconfigureItems() {
        ...
        guard let textA1 = (mockProvider.sectionControllerA.context.cellForItem(CompoundItem(.A, .A1)) as? LabelCell)?.label.text,
              let textA2 = (mockProvider.sectionControllerA.context.cellForItem(CompoundItem(.A, .A2)) as? LabelCell)?.label.text else {
            XCTFail("unexpected cell")
            return
        }
    
        // Reconfigure item A1
        context.reconfigure(items: [CompoundItem(.A, .A1)], invalidateLayout: false) { cell in
        ...
        guard let textA1_1 = (mockProvider.sectionControllerA.context.cellForItem(CompoundItem(.A, .A1)) as? LabelCell)?.label.text,
              let textA2_1 = (mockProvider.sectionControllerA.context.cellForItem(CompoundItem(.A, .A2)) as? LabelCell)?.label.text else {
            XCTFail("unexpected cell")
            return
        }
        ...
        // Reconfigure item A2
        context.reconfigure(items: [CompoundItem(.A, .A2)], invalidateLayout: true) { cell in
            let cell = cell as? LabelCell
            cell?.label.text = updated
        }
        mockManager.collectionView.layoutIfNeeded()
        guard let textA1_2 = (mockProvider.sectionControllerA.context.cellForItem(CompoundItem(.A, .A1)) as? LabelCell)?.label.text,
              let textA2_2 = (mockProvider.sectionControllerA.context.cellForItem(CompoundItem(.A, .A2)) as? LabelCell)?.label.text else {
            XCTFail("unexpected cell")
            return
        }
    ...
    func testRemoveItem() {
        ...
        // Remove item A1
        context.delete(items: [CompoundItem(.A, .A1)])
        mockManager.collectionView.layoutIfNeeded()
    
        // Verify that the item was removed
        let snapshot = context.currentSnapshot()
        XCTAssertFalse(snapshot.itemIdentifiers.contains(CompoundItem(.A, .A1)))
        XCTAssertEqual(snapshot.itemIdentifiers, [CompoundItem(.A, .A2)])
    ...
    func testPerformUpdates() {
        ...
        XCTAssertEqual(snapshot.itemIdentifiers(inSection: .A), [CompoundItem(.A, .A1), CompoundItem(.A, .A2)])
        ...
        XCTAssertEqual(snapshot.itemIdentifiers(inSection: .A), [CompoundItem(.A, .A1), CompoundItem(.A, .A2)])
        XCTAssertEqual(snapshot.itemIdentifiers(inSection: .B), [CompoundItem(.B, .B1), CompoundItem(.B, .B2)])
    }
    ...
    func testPerformUpdatesWithForceReload() {
        ...
        var snapshot = mockManager.diffableDataSource.snapshot()
        XCTAssertEqual(snapshot.sectionIdentifiers, [.A])
        XCTAssertEqual(snapshot.itemIdentifiers(inSection: .A), [CompoundItem(.A, .A1)])
        ...
        XCTAssertEqual(snapshot.sectionIdentifiers, [.A, .B])
        XCTAssertEqual(snapshot.itemIdentifiers(inSection: .A), [CompoundItem(.A, .A1)])
        XCTAssertEqual(snapshot.itemIdentifiers(inSection: .B), [CompoundItem(.B, .B1)])
    ...
    func testPerformUpdatesForSection() {
        ...
        snapshot = mockManager.diffableDataSource.snapshot()
        XCTAssertEqual(snapshot.sectionIdentifiers, [.A, .B])
        XCTAssertEqual(snapshot.itemIdentifiers(inSection: .A), [CompoundItem(.A, .A2)])
        XCTAssertEqual(snapshot.itemIdentifiers(inSection: .B), [CompoundItem(.B, .B1)]) // Ensure section B is not changed
    }
    ...
    func testPerformUpdatesForSectionAndUpdateItems() {
        ...
        XCTAssertNotNil(mockProvider.sectionControllerA.sectionLayoutCalled)
        XCTAssertEqual(mockProvider.sectionControllerA.didEndDisplayingCellCalled?.sectionItem.item, Item.A1)
        XCTAssertEqual(mockProvider.sectionControllerA.willDisplayCellCalled?.sectionItem.item, Item.A2)
