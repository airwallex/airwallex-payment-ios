...
class BillingInfoCellViewModelTests: XCTestCase {
    ...
    func testInit_PlaceholdersReflectPrefilledCountry() {
        mockAddress.countryCode = "JP"
        let viewModel = BillingInfoCellViewModel(
            itemIdentifier: mockIdentifier,
            prefilledAddress: mockAddress,
            reusePrefilledAddress: true,
        ...
        XCTAssertEqual(viewModel.stateConfigurer.placeholder, AddressFieldLabel.state(for: "JP"))
        XCTAssertEqual(viewModel.cityConfigurer.placeholder, AddressFieldLabel.city(for: "JP"))
        XCTAssertEqual(viewModel.zipConfigurer.placeholder, AddressFieldLabel.postcode(for: "JP"))
    }

    func testInit_PlaceholdersReflectDefaultCountryCode() {
        let viewModel = BillingInfoCellViewModel(
            itemIdentifier: mockIdentifier,
            prefilledAddress: nil,
            defaultCountryCode: "US",
        ...
        XCTAssertEqual(viewModel.stateConfigurer.placeholder, AddressFieldLabel.state(for: "US"))
        XCTAssertEqual(viewModel.cityConfigurer.placeholder, AddressFieldLabel.city(for: "US"))
        XCTAssertEqual(viewModel.zipConfigurer.placeholder, AddressFieldLabel.postcode(for: "US"))
    }

    func testSelectedCountry_UpdatesPlaceholdersAndReconfiguresFields() {
        var reconfiguredIdentifiers: [String] = []
        let viewModel = BillingInfoCellViewModel(
            itemIdentifier: mockIdentifier,
            prefilledAddress: nil,
            reusePrefilledAddress: false,
            countrySelectionHandler: {},
            toggleReuseSelection: {},
            cellReconfigureHandler: { id, _ in reconfiguredIdentifiers.append(id) }
        )

        XCTAssertEqual(viewModel.stateConfigurer.placeholder, AddressFieldLabel.state(for: nil))
        XCTAssertEqual(viewModel.cityConfigurer.placeholder, AddressFieldLabel.city(for: nil))
        XCTAssertEqual(viewModel.zipConfigurer.placeholder, AddressFieldLabel.postcode(for: nil))

        reconfiguredIdentifiers.removeAll()
        let ie = AWXCountry()
        ie.countryCode = "IE"
        viewModel.selectedCountry = ie

        XCTAssertEqual(viewModel.stateConfigurer.placeholder, AddressFieldLabel.state(for: "IE"))
        XCTAssertEqual(viewModel.cityConfigurer.placeholder, AddressFieldLabel.city(for: "IE"))
        XCTAssertEqual(viewModel.zipConfigurer.placeholder, AddressFieldLabel.postcode(for: "IE"))
        // state + city + zip + country all reconfigure on country change
        XCTAssertEqual(reconfiguredIdentifiers.filter { $0 == mockIdentifier }.count, 4)

        let hk = AWXCountry()
        hk.countryCode = "HK"
        viewModel.selectedCountry = hk
        XCTAssertEqual(viewModel.stateConfigurer.placeholder, AddressFieldLabel.state(for: "HK"))
        XCTAssertEqual(viewModel.cityConfigurer.placeholder, AddressFieldLabel.city(for: "HK"))
        XCTAssertEqual(viewModel.zipConfigurer.placeholder, AddressFieldLabel.postcode(for: "HK"))
