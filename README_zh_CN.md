...
| 选项 | 描述 | 支付列表 | 仅卡支付 |
|------|------|----------|----------|
| [UI 集成 - HPP（托管支付页面）](#ui-集成---hpp托管支付页面) | 启动完整的、由 SDK 管理的支付流程，包含预构建的支付方式选择、卡输入和结账页面。支持自定义主题和深色模式。**推荐大多数场景使用。** | <img src="Screenshots/hpp_tab.png" width="300" alt="HPP - 多种支付方式"> | <img src="Screenshots/card_payment_with_billing.png" width="300" alt="HPP - 仅卡支付"> |
...
| [Low-level API 集成](#low-level-api-集成) | 使用 SDK 的核心 API 构建完全自定义的支付 UI。直接访问支付方式检索、卡片令牌化、支付确认和 consent 管理。 | 您的自定义 UI | 您的自定义 UI |
